...
const findShortestPath = (startRow, startCol, grid) => {
...
for(z = 0; z < gridComp.length; z++){
  for(j = 0; j < gridComp[0].length; j++){
    //If current node is an a, use it as a starting point
    if(gridComp[z][j] === 1){
      grid = Array(gridComp.length).fill("").map(y => Array( gridComp[0].length).fill(""));
      grid[END_ROW][END_COL] = "Cheese";
      let path = findShortestPath(z, j, grid);
      path ? paths.push(path.length): "";
      //re-initialize path
      path = 0;
    }
  }
}
console.log(paths.sort((a, b) => {return a - b}));
//318?
