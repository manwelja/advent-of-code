...
function initializeMonkeys(dirs) {
  ...
  dirs.forEach((step) => {
    switch(step[0].toLowerCase()) {
      case  "monkey":
        currMonkey = step[1].replace(":", "");
        monkeys[currMonkey] = {};
        monkeys[currMonkey].inspections = 0;
        break;
      case "starting":
        monkeys[currMonkey].stack = step.slice(2);
        break;
      case "operation:":
      ...
      for(let j = 0; j < currDirections.length; j++){
        ...
        switch(step[0].toLowerCase()) {
          ...
          case "operation:":
            ...
            let newOp = monkeys[currMonkey].operation.map(x => x === "old" ? monkeys[currMonkey].stack[0].replace(",", "") : x);
            newWorryLevel = Math.floor(BigInt(eval(newOp.join(""))) / BORED_DIV);
            break;
          case "test:":
            ...
            //check divisibility and add element (with new worry level) to recipient monkey's stack
              newWorryLevel % step.slice(-1) === 0 ?
              monkeys[monkeys[currMonkey].passPass].stack.push(newWorryLevel.toString()) :
              monkeys[monkeys[currMonkey].failPass].stack.push(newWorryLevel.toString());
            break;
          default:
            break;
        }
