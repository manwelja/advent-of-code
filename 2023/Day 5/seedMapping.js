...
try {
    if (fs.existsSync(path)) {
      ...
      fileArray.forEach((element) => {
        if(element.indexOf('seeds: ') >= 0) {
          ...
          seedNumbers.forEach((element) => almanacList.push(new Almanac(element, 0, 0, 0, 0, 0, 0, 0)));
        }
        ...
        // let dest = numberSets[0];
        // let iterator = numberSets[2];
        let [dest, source, iterator] = numberSets;
    ...
    console.log(almanacList.filter((element) => seedNumbers.includes(element.seed)).sort((a, b) => a.location - b.location).shift())
